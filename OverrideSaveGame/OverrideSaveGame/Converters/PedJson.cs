...
namespace OverrideSaveGame.Converters
{
    public class PedJson
    {
        ...
        public PedJson(Ped ped)
        {
            if (!isValidModel(ped.Model))
            {
                throw new Exception("Cannot use PedJson class with FreemodeMale01 and FreemodeFemale01 model.");
            }
            _ped = ped;
            Update();
        }
        ...
        public static PedJson FromJson(string json, bool autoSpawn = false)
        {
            PedJson newPedJson = JsonConvert.DeserializeObject<PedJson>(json);
